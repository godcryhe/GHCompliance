...
if(json.status){var url=$('#createNovelBtn').attr('data-url');if(url){location.href=url;}}else{_dialog.alert(json.info||'操作失败，请稍后再试');}},error:function(){_dialog.alert('操作失败，请稍后再试');},complete:function(){_isChecking=0;}});}
...
{var lockTime=$(obj).attr('data-ltime');var totalWords=$(obj).attr('data-words');var hasLast=$(obj).attr('data-last');if(hasLast==1){_dialog.alert('距离上次申诉解禁未满30天，无法申诉');return;}
if(totalWords<5000){_dialog.alert('作品正文字数满5000字后方可申诉解禁');return;}
